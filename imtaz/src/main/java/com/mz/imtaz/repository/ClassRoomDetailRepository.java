...
public interface ClassRoomDetailRepository extends JpaRepository<ClassRoomDetail, Integer> {
	...
	@Query("Select a from ClassRoomDetail a where a.recordUtility.statusFlag = true order by a.classRoom, a.name, a.teacher.name asc")
	List<ClassRoomDetail> findAllWithOrder();

	@Query("Select a from ClassRoomDetail a where a.recordUtility.statusFlag = true and a.classRoom = :classRoom order by a.classRoom, a.teacher.name asc")
